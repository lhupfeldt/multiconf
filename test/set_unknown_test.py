...
def test_setattr_strict_bad():
    with raises(ConfigException) as exinfo:
        with strict_project(prod2, ef2_prod_pp) as sp:
            sp.c = 1            
    assert replace_ids(str(exinfo.value)) == _setattr_strict_bad_expected_root
    ...
    with raises(ConfigException) as exinfo:
        with strict_project(prod2, ef2_prod_pp) as sp:
            with StrictItem() as si:
                si.z = 1
    assert replace_ids(str(exinfo.value)) == _setattr_strict_bad_expected_nested
    ...
    with raises(ConfigException) as exinfo:
        with strict_project(prod2, ef2_prod_pp) as sp:
            with RepeatableStrictItem('a') as rsi:
                rsi.z = 1
    assert replace_ids(str(exinfo.value)) == _setattr_strict_bad_expected_repeatable_nested
...
def test_setunknown_strict_bad():
    with raises(ConfigException) as exinfo:
        with strict_project(prod2, ef2_prod_pp) as sp:
            sp.setattr('b?', default=1)
    assert replace_ids(str(exinfo.value)) == _setunknown_strict_bad_expected_root

    with raises(ConfigException) as exinfo:
        with strict_project(prod2, ef2_prod_pp) as sp:
            with StrictItem() as si:
                si.setattr('y?', prod=1, default=2)
    assert replace_ids(str(exinfo.value)) == _setunknown_strict_bad_expected_nested

    with raises(ConfigException) as exinfo:
        with strict_project(prod2, ef2_prod_pp) as sp:
            with RepeatableStrictItem('b') as rsi:
                rsi.setattr('y?', prod=1, default=2)
    assert replace_ids(str(exinfo.value)) == _setunknown_strict_bad_expected_repeatable_nested
