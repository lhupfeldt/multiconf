...
def test_builder_ref_env_attr_and_override():
    ...
    assert not hasattr(cr, 'a')  # Note: in pre v6 cr.aa == 19
    assert cr.X.aa == 1  # Note: in pre v6 cr.X.aa == 16
    ...
    assert not hasattr(cr, 'a')  # Note: in pre v6 cr.aa == 5
    assert cr.X.aa == 2
    ...
    cr = ef2_pp_prod.config(pp2)
    assert not hasattr(cr, 'a')  # Note: in pre v6 cr.aa == 7
