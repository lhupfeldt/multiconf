...
def test_env_directly_in_list(capsys):
    assert ef.select_env_list(dev1, [g_dev12_3, dev1, g_dev12], [g_dev12, dev2, dev3]) == 1
    assert ef.select_env_list(dev1, [g_dev12_3, dev3, dev2], [dev1, g_dev12_3]) == 2
    assert ef.select_env_list(dev1, [g_dev12_3, dev1, g_dev12], []) == 1
    assert ef.select_env_list(dev1, [], [dev1, g_dev12_3]) == 2
...
def test_env_in_list_through_group(capsys):
    assert ef.select_env_list(dev1, [g_dev12_3, g_dev12], [g_dev23, dev2, dev3]) == 1
    assert ef.select_env_list(dev1, [g_dev23, dev2, dev3], [g_dev12_3]) == 2
    assert ef.select_env_list(dev1, [g_dev12_3], [g_dev12, dev2, dev3]) == 2
...
def test_env_directly_in_list_ambiguous():
    with raises(AmbiguousEnvException) as exinfo:
        ef.select_env_list(dev1, [dev1], [dev1])

    assert "Ambiguous env select for 'Env('dev1')'." in str(exinfo.value)
    assert exinfo.value.ambiguous == [dev1, dev1]

    with raises(AmbiguousEnvException) as exinfo:
        ef.select_env_list(dev1, [g_dev12_3, dev1, g_dev12], [g_dev12, dev2, dev3, dev1])

    assert "Ambiguous env select for 'Env('dev1')'." in str(exinfo.value)
    assert exinfo.value.ambiguous == [dev1, dev1]

    with raises(AmbiguousEnvException) as exinfo:
        ef.select_env_list(dev3, [g_dev12_3, dev3, dev2], [dev1, g_dev12_3, dev3])
...
def test_env_in_list_through_same_group_ambiguous():
    with raises(AmbiguousEnvException) as exinfo:
        ef.select_env_list(dev1, [g_dev12_3, g_dev12], [g_dev12, dev2, dev3])

    assert "Ambiguous env select for 'Env('dev1')'." in str(exinfo.value)
    assert exinfo.value.ambiguous == [g_dev12, g_dev12]

    with raises(AmbiguousEnvException) as exinfo:
        ef.select_env_list(dev1, [g_dev12_3], [dev2, dev3, g_dev12_3])
...
def test_env_in_list_through_different_groups_ambiguousxo():
    with raises(AmbiguousEnvException) as exinfo:
        res = ef.select_env_list(dev1, [g_dev12_3, g_dev13], [g_dev12, dev2, dev3])
        print("dev1.lookup_order", dev1.lookup_order)
        print(res)

    with raises(AmbiguousEnvException) as exinfo:
        res = ef.select_env_list(dev1, [g_dev12_3, g_dev12], [g_dev13, dev2, dev3])
